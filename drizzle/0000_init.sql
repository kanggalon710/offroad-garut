...
CREATE TABLE `meeting_points` (
	...
	CONSTRAINT `meeting_points_latitude_check` CHECK(`meeting_points`.`latitude` between -90 and 90),
	CONSTRAINT `meeting_points_longitude_check` CHECK(`meeting_points`.`longitude` between -180 and 180)
);
--> statement-breakpoint
...
CREATE INDEX `idx_meeting_points_location` ON `meeting_points` (`latitude`,`longitude`);--> statement-breakpoint
CREATE INDEX `idx_package_galleries_package_id` ON `package_galleries` (`package_id`);--> statement-breakpoint
CREATE INDEX `idx_packages_search` ON `packages` (`name`);--> statement-breakpoint
