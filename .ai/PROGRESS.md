...
**Perubahan:**
- `FormPin` dapat kolom "PIN sekarang" yang muncul saat akun memang sudah punya PIN, dan mengirimkannya ke server. Ditambah kartu "Ganti PIN" di keadaan normal, karena sebelumnya satu-satunya jalan masuk ke layar itu adalah alur wajib-ganti yang cuma jalan sekali seumur akun.
- `/api/upload` memakai `isStaff(toRole(...))`. `src/test/pengecekan-peran.test.ts` baru memindai seluruh `src/` dan gagal kalau ada berkas yang membandingkan peran secara mentah lagi. Penjaga itu dibuktikan menyebut berkas yang salah saat cacatnya disisipkan ulang.
- Migrasi `0008`: tabel `jeep_galleries` dan kolom `jeeps.tampil_publik` (default false). Unggah beberapa foto sekaligus di kartu Jeep, dengan galeri yang bisa diklik untuk memperbesar dan tombol panah.
- `PackageGalleryCarousel` diganti nama jadi `GaleriCarousel` dan prop `packageName` jadi `namaObjek`. Komponennya dipakai ulang apa adanya oleh galeri Jeep dan seksi armada publik, jadi tidak ada komponen kembar. Tombol panahnya dinaikkan dari 40px ke 44px dan tombol Zoom dari 30px ke 44px, memenuhi target sentuh minimal yang selama ini dilanggar diam-diam di halaman paket.
- `hapusBerkasUnggahan()` di `src/lib/upload.ts`: menghapus foto ikut menghapus berkasnya di disk, dengan dua penjaga jalur supaya string dari database tidak bisa menghapus berkas di luar direktori unggahan.
- Seksi "Armada" di beranda, hanya memuat unit yang ditandai publik DAN punya foto.
- Migrasi `0009`: tabel `jeep_maintenances` dengan riwayat servis, biaya, dan tanggal servis berikutnya. Dashboard memperingatkan unit yang jadwalnya sudah dekat atau lewat. Unit tanpa `servis_berikutnya` tidak pernah diperingatkan.
- Halaman `/laporan` dengan empat bagian: papan jadwal harian per unit (lengkap dengan add-on yang harus disiapkan, pesanan yang belum dapat Jeep, dan unit yang menganggur), utilisasi armada, performa paket dan add-on, serta pola hari dan jam. Perhitungannya dipisah ke `src/lib/laporan.ts` supaya bisa dites tanpa database.
- Batang perbandingan dibuat sendiri dengan `div`, tanpa pustaka grafik baru, dan angkanya selalu ditulis di sebelah batang.
**File:** src/components/admin/pembaruan-client.tsx, src/app/api/upload/route.ts, src/lib/upload.ts, src/lib/laporan.ts, src/lib/db/schema.ts, src/lib/constants.ts, src/lib/rute-privat.ts, drizzle/0008_jeep_galleries.sql, drizzle/0009_jeep_maintenance.sql, src/server/routers/laporan.ts, src/server/routers/admin.ts, src/server/routers/booking.ts, src/server/routers/_app.ts, src/app/(admin)/laporan/page.tsx, src/components/admin/laporan-client.tsx, src/components/admin/bar-ringkas.tsx, src/components/admin/jeep-galeri.tsx, src/components/admin/jeep-servis.tsx, src/components/admin/dashboard-client.tsx, src/components/admin/admin-header.tsx, src/components/admin/master-data/jeeps-manager.tsx, src/components/domain/galeri-carousel.tsx, src/components/landing/armada.tsx, src/app/(public)/(beranda)/page.tsx, src/app/(public)/paket/[slug]/page.tsx, src/middleware.ts, src/test/pin-ganti.test.ts, src/test/pengecekan-peran.test.ts, src/test/hapus-unggahan.test.ts, src/test/laporan.test.ts
**Catatan:** Cacat PIN direproduksi dulu di database lokal (super_admin dengan PIN bawaan dan wajib-ganti aktif) sebelum diperbaiki, lalu diverifikasi di peramban: PIN lama salah ditolak dengan pesan jelas, PIN lama benar membuka halaman pembaruan. Unggah sebagai `super_admin` yang sebelumnya 403 kini berhasil. Menghapus foto terbukti menghapus berkasnya (3 jadi 2 di database dan di disk). Angka laporan dicocokkan silang: 16 perjalanan, paket 6+5+5=16, sebaran hari berjumlah 16, slot 8+8=16, dan unit yang tidak pernah dipakai tetap tampil 0 persen alih-alih hilang. Harga add-on dinaikkan 350.000 jadi 999.000 lalu laporan diperiksa: tetap Rp1.750.000, snapshot bekerja. Tiga penjaga tes dibuktikan benar-benar gagal saat regresinya disisipkan. 360, 768, dan 1280px bersih untuk seluruh halaman laporan. `typecheck` dan `lint` bersih, `test` 150 lolos, `build` sukses.
